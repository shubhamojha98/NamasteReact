...
const Body = () => {
  ...
  const filterTopRated = () => {
    ...
    const sortedList = filteredList?.sort((a, b) => b.info.avgRating - a.info.avgRating);
    ...
    <>
      <div className="body">
        <div className="search">
          <input type="search" className="search-input" placeholder="Search a restaurant you want..." value={searchText} onChange={(e) => {
            setSearchText(e.target.value);
          }} />
          <button className="search-btn" onClick={() => {
            const filteredListByName = listofResturant.filter((res) => res.data.name.toLowerCase().includes(searchText.toLowerCase()));
